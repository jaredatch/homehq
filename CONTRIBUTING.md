...
- If it touches a personal board, the rules are in [docs/architecture.md](docs/architecture.md#boards): a board only overrides top-level settings and never supplies a base one; nothing on a touch panel takes text focus (the drawn keyboard writes into a `div`, because an `<input>` summons a platform keyboard the Pi can't show); and `/api/calendar` decides what a board may see from the session, not from the request.
