...
- New config keys get a default, a line in `lib/config/types.ts`, validation in `lib/config/index.ts`, a row in [docs/configuration.md](docs/configuration.md), and an entry in `data/config.example.json`. A `display` key also has to come through the board merge in `lib/config/boards.ts`, or a personal board will never see it.
- If it touches a personal board, the rules are in [docs/architecture.md](docs/architecture.md#boards): a board only overrides top-level settings and never supplies a base one; nothing on a touch panel takes text focus (the drawn keyboard writes into a `div`, because an `<input>` summons a platform keyboard the Pi can't show); and the API decides what a board may see and change from the session, not from the request.
...
If you find something security-relevant (the PIN gate, per-board PINs, the OAuth flow, the session cookie, or a way for one board to see another board's calendars), use GitHub's private vulnerability reporting on this repo rather than a public issue.
