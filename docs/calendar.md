...
`DayPopover` shows **only the hidden events**, not the day. Month view's shows the day in full because a month cell holds two or three chips and the rest is genuinely unseen; a week column already shows a dozen, so listing them again answers a question nobody asked and covers the neighbouring days to do it. "+3 more" opens three rows. There are no band bars in it either: an all-day event, and a timed one running past midnight, always draw in the band, so nothing there can ever be behind a `+N more`.

It is not `MonthDayPopover`, and it does not share `popoverLayout()`. `.mon-pop` is sized in `em` against `.mon-calendar`'s `clamp()` font size and draws month chips; on the wall it would land at the wrong scale and in a different visual language from the column right behind it. The wall's is `rem` and renders real `EventItem` rows, so the card reads as that cell continued. Placement is `dayPopoverBox()`: centred on the column like month view, but anchored to the **button's own edge** and growing into whichever side has more room — upward, in practice, since a week column is full height and its `+N more` is the last thing in it. Month view's rule of reserving half the region to grow into opened a three-row card ~400px above the button that summoned it. It mounts inside `.cal-grid`, never inside `.cal-weeks` — that's `overflow: clip` and holds the measurement layer. Rows need `flex-shrink: 0` or a long day compresses every row to fit instead of scrolling. Esc, a click outside, and a resize all close it, and it shares `display.expandResetSeconds` with the toggle rather than growing the config a near-duplicate key.
...
**The way out is a house glyph at the far left of the footer**, on every full-screen view, from the shared `PersonalViewFooter`. It used to be a "Close" text link at the _end_ of each view's own action list, which put the exit somewhere different depending on how many actions that view had, and would have moved it again the first time one gained another. The glyph is Font Awesome's `house`, hand-inlined rather than routed through `title-icons.ts`: that machinery is server-only and exists so a glyph can change with a `config.json` edit, and this one is fixed chrome. A new full-screen view gets the button by rendering this footer. The three-column board itself has none, since you are already there.

Sizing overrides live under `.pb-view` in `personal.css`; `calendar.css` and `month.css` are untouched, which is what keeps the wall unmoved by construction. The chrome inside those overrides is deliberately the _family board's_ language, not a second one: `.pb-view-title` takes the clock's presence and `.pb-view-range` the date line beside it, the paging chevrons are month view's outlined `.mon-nav-btn` grown to a finger's target, and the footer's link and sync colours are the wall footer's exactly. Only the sizes differ, because a thumb needs more than `--text-xs`.
