...
12. **The personal board owns its own chrome, and shares only presentational grids.** It renders `WeekRow` and `MonthWeek` unchanged (one definition of a week and a month house-wide) plus the shared measuring in `week-metrics.ts` / `month-metrics.ts`, and it shares the write _routes_. It never shares a form or a footer: bending `EventModal` or `CalendarFooter` around a second set of constraints is how the family board gets broken. And nothing there takes text focus: the drawn keyboard writes into a `div`, because an `<input>` invites a platform keyboard the Pi can't show — a physical keyboard reaches it through a window listener, never through focus. No native date or time control either: Chromium opens those from a mouse-sized icon and draws them in the DEVICE's locale (a Pi on Raspberry Pi OS's en_GB default showed 22/09/2026 and 22:00), so dates and times go through the drawn `DatePicker` / `TimePicker`.
    **Separate components, one design language.** Owning the chrome is not licence to invent a second scheme. `--blue-600` is the app's one commit colour; footer links are `--gray-500` and the sync label `--gray-600` on both boards; a view header is the clock's type and month view's outlined chevrons. Only sizes differ, because a thumb needs more than `--text-xs`. The per-board `accent` key was removed on 2026-09-02 for exactly this reason — it had leaked from identity into every primary button, and a form on a bedroom panel stopped looking like a form on the kitchen wall.
...
- A day popover mounts in `.cal-grid` / `.mon-calendar`, never in `.cal-weeks` / `.mon-grid` — those are `overflow: clip` and hold the measuring. The wall and month view each own their popover and share nothing, not even the clamping math (`dayPopoverBox` vs `popoverLayout()`); `.mon-pop` is `em` against month view's `clamp()` font size and is not portable.
...
- An icon drawn inside a title must be `display: inline-block`. `base.css` resets `img, svg, video, canvas` to `display: block` so icons don't pick up baseline whitespace, and a block inside a title takes a line of its own: `.cal-band-bar` grew from 29.5px to 47px against a 29.5px spacer and the whole all-day overlay slid off its lanes. `.cal-title-icon` is 0.875em at `vertical-align: -0.1em`, which fits inside the line box Inter already makes at every leading here. Verified equal to the spacer at 1920x1080 and at the wall's 43.2px root.
