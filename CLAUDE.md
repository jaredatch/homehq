...
- **Wall-default byte-for-byte** — a new calendar feature must leave the _default_ wall render pixel-identical **for data that doesn't use it**; prove it with a normalized 4K DOM diff of the week grid, taken against a database containing none of the feature's data. This is why month view is a **separate component** and the filter's off state returns the **same `events` reference** — never a second mode bolted onto the delicate `CalendarGrid`. A feature that intentionally changes the default render (shared events collapsing two chips into one) must **name that delta explicitly** and stay byte-identical everywhere else. "It's supposed to look different" is never a reason to skip the diff.
...
- **Shared events (v1.3)** — one event that applies to **two people** (Maddie _and_ Eleanor; Jared _and_ Sam). Stored as **one real Google event per calendar**, each stamped with the same id in `extendedProperties.private.homehqGroup` and mirrored into a nullable `group_id` column (migration `003`). Deliberately **not** Google's attendee/invite trick: that creates separate event resources anyway (so the read side still dedupes), lands asynchronously (breaking confirmed-then-cached), and forces a fake organizer/guest hierarchy. Because each row still belongs to exactly one calendar, **the sync needed zero changes** — the per-calendar wholesale replace stays correct and the stamp rides along. The modal's calendar `<select>` is a **checkbox picker capped at `MAX_GROUP_CALENDARS` (2)** (`CalendarPicker` — see below), live in create _and_ edit; editing membership runs a **three-way diff** (patch kept · insert added · delete removed, deletes **last** and skipped entirely when nothing was written, so a failed patch can't destroy the other copy). Partial failures **keep what landed and report what didn't** — no rollback. On the read side `mergeGroups()` runs **after `filterEvents`** (that order is why filtering to Maddie renders a shared event in _her_ color for free) and collapses matching copies into one chip carrying `groupCalendarIds`; **drift un-merges** — copies edited apart outside HomeHQ simply render separately again. Both return the **same array reference** when idle, so an unshared board is byte-identical. Paint lives in `event-paint.ts`, with every knob a CSS var in `styles/calendar.css`: forward-slash stripes on the all-day fill (`--cal-stripe` period, in `em` so it scales; `--cal-stripe-angle` **135deg** — a gradient angle names the gradient _line_ and the bands run perpendicular, so 45deg renders as a backslash), a much tighter banded `--cal-stripe-accent` on the timed event's ~1px accent sliver (a 50/50 split there reads as one color), and **two whole dots** on a month chip rather than one split circle (a split is a muddy blob when the hues are close — Maddie pink beside Eleanor purple). The all-day fill is the only surface where text sits _on_ the color, so it gets `.cal-band-label`, a dark scrim that frees the fill from `contrastText` having to pick one answer for two colors. **The scrim must stay `display: inline`** — `inline-block` sizes the line box from its own padded height and makes a shared bar taller than its invisible `.cal-band-spacer`, drifting the whole band overlay off the lanes each day reserves. Its vertical padding (`--cal-scrim-pad-y`) must stay **symmetric**: ink sits 4.24px below the pill's top and the pill 4.26px below the bar's top, so the two are already concentric and equal padding grows it around a centered point. Re-measure if the band's font-size or line-height changes.
- **Calendar picker (v1.3.x)** — the event modal's calendar chooser (`CalendarPicker`): a collapsed field showing the chosen calendars as **tokens with a ✕**, opening a checkbox list. It replaced a wrapping grid of always-visible pills, which read fine at four calendars and at eight wrapped to three rows — 157px, **22% of the modal's whole height**, growing with every calendar added. Collapsed it is one row at any count. The trade is one click to open, and no color-at-a-glance on a _blank_ form; an event being **edited** still shows its calendars as tokens without opening anything. Reaching the cap **auto-closes** the list (a shared event is open · click · click). The list is **portaled to `<body>` as `position: fixed`** — `.cal-modal` is `overflow-y: auto`, which clips **both** axes, so a dropdown anchored inside it would be cut off at the modal's edge; expanding inline instead would shove the date fields down, the exact jump `1012837` removed from the all-day toggle. JS sets only left/width/top-or-bottom/max-height (it flips above the field when below is cramped); the gap and every other metric stay **rem**, so the whole thing scales on the wall. Open state lives in `EventModal`, not the picker, because three of the modal's behaviors have to **peel it first**: **Esc**, a **backdrop click**, and **Enter** (which otherwise saves — inside the list it toggles the focused row). Focus moves to the first row on open via a **callback ref**, not an effect: the portal mounts one commit after the open flip (it waits on the measured box), so an effect would run while the ref is still null.
...
- **Event creation + edit/delete (v1.1 · v1.1.x, opt-in)** — full write access to Google Calendar (create, edit, delete), gated by `config.google.calendarAccess` (`readonly` **default** | `readwrite`). Read-only deployments are unchanged: only the `calendar.readonly` scope is requested, the `+ Add event` UI is hidden, events are **inert** (not clickable), and the write routes (`POST /api/calendar/create` · `/update` · `/delete`) 403. In `readwrite`, the OAuth scope becomes `calendar.events` (re-consent at `/setup` to upgrade an existing token); a footer **"+ Add event"** button and **clicking any event** open `EventModal` (one component, `mode: 'create' | 'edit'`). All three write routes are **confirmed-then-cached**: call Google → on success `normalizeEvent` + `upsertEvent` (a single-event upsert that does **not** wipe the calendar, unlike the sync's `upsertCalendarEvents`), or `deleteEvent` for a delete, write straight into SQLite so the change shows on the next poll without waiting for the 5-min sync. **Edit** reuses the modal pre-filled, saves via Google `events.patch` (null-clears the start/end sibling so an all-day↔timed toggle is clean). **Delete** is a named confirm step (shows title + when). **Recurring occurrences are blocked** (edit + delete) with a "manage in Google Calendar" notice — the sync expands recurrences (`singleEvents:true`), so each row is one occurrence; a `recurring_event_id` column (migration `002`, carried by `normalizeEvent`) flags them, and the routes 409 on a recurring row as defense in depth. Shared field validation lives in `lib/calendar/event-timing.ts` (`parseTiming` + UTC date math). Fields: title · calendars (**`CalendarPicker`, no default — at least one must be chosen**; see Shared events and Calendar picker) · date · start/end times or all-day · location · notes. **All-day events carry a start AND end date** so a multi-day span can be set or resized. The end date shown is the **inclusive last day** — Google's `end.date` is **exclusive**, so the UI subtracts a day on read and `parseTiming`'s caller adds one on write; skip that conversion and every multi-day event silently shifts. One rule governs the pair: **if the end equals the start, the end follows the start; otherwise the edges are independent** — read live off the current field values, so a new event (and a single-day one, where moving the date means moving the event) tracks, while a multi-day event just grows or shrinks. An inverted range blocks Save rather than silently clamping. Timed events stay **same-day**: the grid renders an event crossing midnight only on its start day, so the UI doesn't offer it. The date/start/end row holds three **native** controls whose widths the browser dictates (~23.7rem between them), which is why `.cal-modal` is **27rem** and nothing in that row may shrink — a squeezed `input[type=date]` doesn't ellipsize, it **clips its own value** (`08/12/202` reads as a date and isn't one), so `.cal-field--date` grows but never shrinks. Measure before narrowing either. The modal auto-closes after `config.display.createFormResetSeconds` idle (default 120; wall-never-sticks). `isCalendarWriteEnabled()` (in `lib/config`) is the single gate for the scope, the routes, the button, and click-to-edit.
...
├── calendar/                         # CalendarView (week/month switch + auto-revert), CalendarGrid (wall: measures/sizes), WeekRow, EventItem, MonthGrid/MonthWeek/MonthDayPopover (month view), CalendarFooter (shared bottom bar), EventModal (create + edit + delete), CalendarPicker (collapsed calendar chooser), utils (calendar-utils, month-utils)
...
**v1.3:** **shared events** — one event on two calendars (real Google event per calendar + a group stamp), a checkbox calendar picker capped at 2, editable membership, and a merged two-color chip on the board
