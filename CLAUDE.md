...
- **`+N more` cropping** — `CalendarGrid` renders a **hidden measurement layer** (full, uncropped event stacks at the real column width) so it reads each day's **actual** per-event heights (1- vs 2-line titles) instead of assuming a uniform row, then greedy-packs each cell — a cell shows as many events as truly fit. Each cell's available height subtracts **only its own column's band reservation** (`bandHeightFor(laneByColumn[col])`, per-column — not the week's max), so days under no all-day bar spend the reclaimed room on extra events. Track sizing + crop honor a strict priority: (1) today + later days of the **current week** show every event and set the week-0 track height; (2) next/later weeks get the remaining height (maximized); (3) past days of the current week crop last (never steal from next week).
